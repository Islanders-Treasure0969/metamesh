...
### 既存 Concept に Kimball 拡張を追加する場合

既に `dv:hub` 等の DV 拡張を持つ Concept (典型: CBC で先に登録済みの
`Streamer` 等) に Kimball 拡張を追加する場合、`add_concept` の
`extension` 引数に **既存 dv 拡張と新規 kimball 拡張を両方含む list**
を渡す:
...
mcp__metamesh__add_concept(
    concept_id="Streamer",
    pref_label_ja="配信者",
    definition_ja="...",  # 既存 Concept の値を維持して再渡し
    # ... 他の既存値も維持
    extension=[
        # 既存 dv 拡張を維持 (失わせない)
...
**手順:**
1. `mcp__metamesh__query_concept` で既存 Concept の全フィールドを取得
2. dv 拡張 (もしあれば) を確認
3. 上記の形式で `add_concept` を呼び出し、dv + kimball の両方を渡す

これで両 namespace が共存した jsonld が書かれる。同 namespace を複数回
渡した場合は **last-wins** (incremental overlay 用途)。
...
3. **既存 Concept への Kimball 拡張追加**

   既存 Concept (`Streamer` 等) に Kimball 拡張を追加する場合は、
   上記「既存 Concept に Kimball 拡張を追加する場合」セクション参照。
   `extension=[dv, kimball]` の list 形式で両方を一度に渡す。
...
- ❌ **degenerate dimension と通常 Dim を混同** → 低カーディナリティで
   ...
   Fact 行に直接持つ
- ❌ **既存 dv: 拡張を上書きする呼び方** → 上記「既存 Concept に Kimball
   拡張を追加する場合」を参照。`extension=[dv 既存, kimball 新規]` の list
   形式で両方を一度に渡せば共存できる
