...
| 学派 | 中核 | 出典 |
|---|---|---|
| **Classic Kimball** (既定) | 厳格 Star Schema、Conformed Dim 共有、`dim_/fct_` プレフィックス | Ralph Kimball & Margy Ross _The Data Warehouse Toolkit, 3rd ed._ (Wiley, 2013) / Christopher Adamson _Star Schema The Complete Reference_ (2010) |
...
**既定は Classic** (業界実態として `dim_/fct_` プレフィックス + 厳格
...
> ⚠️ **これは MCP ツール呼び出しではなく、Skill が会話で収集すべき設計パラメータ
> の定義**。`star-schema-design` は MCP ツールとして登録されているわけではなく、
> Claude が本 Skill 経由で `mcp__metamesh__query_concept` 等を呼び出して
> ファイルを生成するワークフロー。

| パラメータ | 既定値 | 選択肢 | 説明 |
|---|---|---|---|
| `output_path` | (必須) | 任意のパス | 出力ファイル / ディレクトリ |
| `mode` | `classic` | `classic` / `wide` / `hybrid` | 設計学派 (詳細は次節) |
| `naming` | `kimball_prefix` | `kimball_prefix` / `plain` | テーブル命名 |
| `sk_strategy` | `hash` | `hash` / `identity` / `uuid` | Surrogate key 戦略 |
| `dialect` | `markdown` | `markdown` / `snowflake` / `bigquery` / `postgres` / `dbt_model` / `dbt_snapshot` / `dbt_yaml` | 出力形式 |
| `include_audit_cols` | `True` | bool | audit 列を付ける |
| `include_tests` | `True` | bool | dbt schema.yml の テストを生成 |
...
| 値 | 内容 | いつ選ぶ |
|---|---|---|
| **`classic`** (既定) | 中央 Fact + 周辺 Dim を別テーブル、JOIN 前提 | 伝統的 BI ツール (Tableau / Looker) 接続、Kimball 教科書準拠、Mart 共有 |
| `wide` | Dim を Fact に joined-and-flattened したワイドテーブル | dbt + Snowflake/BigQuery 主体、columnar 圧縮で ストレージコスト低、JOIN 削減で クエリ高速化 |

### `naming`

| 値 | 例 | 採用根拠 |
|---|---|---|
| **`kimball_prefix`** (既定) | `dim_customer`, `fct_orders`, `agg_sales_monthly` | 業界デファクト、Kimball Toolkit / Adamson 標準、企業 BI ガイドライン主流 |
...
| 値 | 実装 | 強み | 弱み |
|---|---|---|---|
| **`hash`** (既定) | `dbt_utils.generate_surrogate_key([...])` (MD5) | 全 dialect 動作、確定的、分散環境対応 | hex 32 文字 |
...
| 値 | 用途 |
|---|---|
| **`markdown`** (既定) | レビュー / ドキュメント、最初に出すべき形式 |
...
| `dbt_model` | dbt model `.sql` 雛形 (incremental / table マテリアライゼーション) |
| `dbt_snapshot` | dbt snapshot `.sql` 雛形 (SCD Type 2 専用) |
| `dbt_yaml` | dbt schema.yml (テスト + ドキュメンテーション) |
...
|---|---|---|---|
| **Type 0** | Retain Original | カラム追加なし、不変 | dbt model + immutable test (不変性テスト) |
| **Type 1** | Overwrite | UPDATE、履歴破棄 | dbt model `materialized: 'table'` または incremental (insert/overwrite 戦略) |
...
| Type 6 | 1+2+3 Combined | current_X + historical_X + valid_from + valid_to | snapshot と dbt model の組合せ |
...
| カラム | 意味 |
|---|---|
| `dbt_valid_from` | snapshot 行が有効化された時刻 |
| `dbt_valid_to` | 無効化時刻 (NULL = current) |
| `dbt_updated_at` | source の `updated_at` 値 |
| `dbt_scd_id` | snapshot 行ごとの一意識別子 |

dbt 1.9+ では `dbt_valid_to_current: '9999-12-31'` で sentinel (番兵) 値設定可。
本 Skill は 既定で sentinel (番兵) 値を採用 (NULL より 範囲クエリが単純)。
...
| `<attribute>...` | 各属性列 | descriptive (記述的属性) |
...
2. **発見**
   ...
   ### 結果の打ち切り (truncated) 検証 ⚠️ 必須

   `mcp__metamesh__query_concept` のレスポンスは件数が多いと
   `limit` で打ち切られ、`truncated: true` が立つ。**打ち切られた状態で
   物理設計を生成すると、一部 Dim / Fact が出力から欠落する** (= 業務
   インパクト直結の bug)。

   フロー:
   1. 上記 SPARQL を `limit` 指定で投げる (既定は本 Skill 内部で
      `100` 程度を上限と仮定)
   2. レスポンスの `truncated` を確認
   3. `truncated == true` なら以下のいずれかを実施:
      - **再投入**: `limit` を倍増して再実行 (例: 100 → 500 → 2000)
      - **絞り込み**: `mode` を分割 (例: `dim` だけ先、`fact` だけ後)
      - **失敗扱い**: ユーザーに「ontology が大きすぎる、`mode` 単位で
        分割実行を」と案内 (推奨)
   4. 全件取得が確認できるまで生成フェーズに進まない

   コード例 (Skill 内部の擬似ロジック):

   ```text
   res = query_concept(sparql=<dim 取得>, limit=500)
   if res["truncated"]:
       raise SkillError(
           "Dim 取得が打ち切られました (limit=500 で truncated=true)。"
           "オントロジーが大きい場合は `dim` のみ・`fact` のみで"
           "分割生成してください。"
       )
   ```
